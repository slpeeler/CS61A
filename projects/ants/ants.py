...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_ant and (isinstance(insect, QueenAnt) == False or (isinstance(insect, QueenAnt) and insect.is_imposter == True)):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if (self.armor - amount) <= 0:
            spot = self.place
            for bee in spot.bees:
                bee.armor -= self.damage
            for bee in spot.bees[:]:
                if bee.armor <= 0:
                    index = spot.bees.index(bee)
                    del spot.bees[index]
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_imposter == True:
            ...
            def double(bug):
                if bug.has_doubled == False:
                    bug.damage *= 2
                    bug.has_doubled = True
            next_place = self.place.exit
            while next_place != None:
                ant = next_place.ant
                if ant != None:    
                    double(ant)
                    if ant.is_container and ant.contained_ant != None:
                        double(ant.contained_ant)
