...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        if check_procedure(scheme_eval(expr.first, env)):
            raise SchemeError('not a scheme procedure')
        else:
            def eval_short(exp):
                return scheme_eval(exp, env)
            fun = scheme_eval(expr.first, env)
            opers = expr.second.map(eval_short)
            return scheme_apply(fun, opers, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('wrong lengths')
        else:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        val = (expressions.second.first, env)
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.second != nil:
                while clause.second.second != nil:
                    scheme_eval(clause.second.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    sym = Pair(bindings.first.first, nil)
    ...
    while bindings != nil:
        ...
        bindings = bindings.second
    check_formals(sym)
    let_frame = env.make_child_frame(sym, vals)
    return let_frame
    # END PROBLEM 15
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            first = exp.first
            second = exp.second
            if first in SPECIAL_FORMS.keys():
                result = SPECIAL_FORMS[first](second, envi)
            else:
                fn = original_scheme_eval(first, envi)
                check_procedure(fn)
                args = second.map(lambda x: original_scheme_eval(x, envi))
                result = scheme_apply(fn, args, envi)
                print(result)
        return result
        # END
