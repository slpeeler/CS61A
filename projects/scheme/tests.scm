...
(begin (+ 3 (* 7 6)) (define x 1) (define y 2) (define (z x y) (x + y)))
; expect z
x
;expect 1
y
...
(define (quadruple x y) (* y (expt x 4)))
;expect quadruple
(quadruple 2 2)
;expect 32

(lambda (x y z) (+ (* y z) x))
; expect (lambda (x y z) (+ (* y z) x))

(and (even? 27) ((lambda (x y z) (/ x y z)) 1 0 0))
;expect #f

(and (odd? 27) ((lambda (x y z) (/ x y z)) 1 0 0))
;expect Error

(or (even? 27) ((lambda (x y z) (/ x y z)) 1 0 0))
;expect Error

(or (odd? 27) ((lambda (x y z) (/ x y z)) 1 0 0))
;expect #t

(define (sum-3 x)
  (if (odd? x)
    (+ x 3)
    (begin (define x (+ x 1)) (sum-3 x))))
;expect sum-3
...
(let ((x (sum-3 s)) (y (sum-3 z))) (sum-3 (+ x y)))
;expect 36

(cond ((= (sum-3 s) 15) 'wrong)
  ((= (sum-3 z) 15) 'wrong)
  ((and (= (sum-3 s) 16) (= (sum-3 z) 16)) 'right!))
;expect right!

'(begin ((define x 1)) x)
;expect (begin ((define x 1)) x)

(cdr (quote (1 2 3 4 5)))
;expect (2 3 4 5)

(begin (sum-3 27) (odd? (sum-3 27)) (even? (sum-3 27)))
;expect #t

(begin ((lambda (x y) (> x y)) 1 2) ((lambda (y x) (> x y)) 1 2))
;expect #t

(define (cuber und) (* und (* und und)))
;expect cuber

cuber
;expect (lambda (und) (* und (* und und)))

(define (test3 test4) (begin (define test4 (+ test4 1)) test4))
;expect test3
...
(define (count-him-up counter num)
  (if (> num 0)
    (begin
      (define counter (+ counter 1))
      (count-him-up counter (- num 1)))
    counter))
;expect count-him-up
...
(define result (lambda () (define a 103) (define b 104) (adder)))
;expect result
...
;;;(let ((x 2)) ((begin (define x (+ x 1)) +) (begin (define x (+ x 2)) 3) (begin (define x (+ x 1)) x)))
;;;;expect 9
