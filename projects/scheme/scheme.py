...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if check_procedure(fn):
            raise SchemeError('not a scheme procedure')
        elif isinstance(fn, MacroProcedure):
            return scheme_eval(fn.apply_macro(rest, env), env)
        else:
            opers = expr.second.map(lambda x: scheme_eval(x, env))
            return scheme_apply(fn, opers, env)
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        sym = Pair(binding.first, sym)
        vals = Pair(scheme_eval(binding.second.first, env), vals)
        bindings = bindings.second
    check_formals(sym)
    return env.make_child_frame(sym, vals)
...
def do_define_macro(expressions, env):
    ...
    elif scheme_symbolp(expressions.first.first) == False:
        raise SchemeError('no')
    else:
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            exp = result.expr
            envi = result.env
            result = original_scheme_eval(exp, envi)
