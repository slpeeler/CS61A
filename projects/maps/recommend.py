...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    closest = [centroid for centroid in centroids if distance(location, centroid) == min([distance(location, c) for c in centroids])] 
    return closest[0]
    # END Question 3
...
def find_centroid(cluster):
    ...
    for i in range(0, len(cluster)):
        lats.append(restaurant_location(cluster[i])[0])
    longs = []
    for i in range(0, len(cluster)):
        longs.append(restaurant_location(cluster[i])[1])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    for k in range(0, len(xs)):
        sxy += (xs[k] - sxx_mean)*(ys[k] - syy_mean)
    b = sxy / sxx
    a = syy_mean - (b * sxx_mean)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    max_pred = predictors[0]
    for i in range(1, len(predictors)):
        if predictors[i][1] > max_pred[1]:
            max_pred = predictors[i]
    return max_pred[0]
    # END Question 8
...
def search(query, restaurants):
    ...
    for item in restaurants:
        if query in restaurant_categories(item):
            result.append(item)
    return result
    # END Question 10
